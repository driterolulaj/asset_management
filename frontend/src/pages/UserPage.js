...
import { TextField, Button, Table, TableRow, TableCell, TableHead, TableBody, Container, Typography } from '@mui/material';
  ...
  return (
    <Container>
      ...
      <div>
        ...
        <Button variant="contained" onClick={handleAddOrUpdateUser}>{editingId ? 'Update User' : 'Add User'}</Button>
      </div>
      <Table>
        <TableHead>
          <TableRow>
          ...
          {users.map(u => (
            <TableRow key={u.id}>
              ...
              <TableCell>
                ...
                <Button color="error" onClick={() => handleDeleteUser(u.id)}>Delete</Button>
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
